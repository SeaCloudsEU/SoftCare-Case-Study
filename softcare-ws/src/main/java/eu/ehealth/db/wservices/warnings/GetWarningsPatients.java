				...
				for (int i = 0; i < sc.length; i++)
				{
					for (int j = 0; j < field.length; j++)
					{
						if (field[j].getName().compareToIgnoreCase(sc[i].getFieldName()) == 0)
						{
							...
							sql += String.format(StorageComponentImpl.operationsMap.get(opnum), "warning." + sc[i].getFieldName(), 
					...
					{
						Integer opnum = new Integer(sc[i].getCompareOp().getCode());
						sql += String.format(StorageComponentImpl.operationsMap.get(opnum), sc[i].getFieldName(), 
								sc[i].getFieldValue1(), sc[i].getFieldValue2());
						sql += " AND ";
					}
				}
