...
public class RegisterUser extends DbStorageComponent<String[], String>
{
	...
	protected String[] dbStorageFunction(ArrayList<String> lo)
	{
		try {
			...
			SystemDictionary.webguiLog("DEBUG", "RegisterUser CALL ... ");
			SystemDictionary.webguiLog("DEBUG", "RegisterUser CALL params : " + userName + " / " + password + " / " + userId);
			...
			if (fu.existUser(userName, 0) != 1) {
				...
				if (password.matches(PSSWD_REG_EXPRESSION)) {
					SystemDictionary.webguiLog("INFO", "VALIDATING PASSWORD : OK");
				}
				else {
					SystemDictionary.webguiLog("ERROR", "VALIDATING PASSWORD ERROR : " + password);
