...
public class GetUserPlannedTasks extends DbStorageComponent<List<Task>, Object>
{
	...
	protected List<Task> dbStorageFunction(ArrayList<Object> lo)
	{
		try
		{
			...
			if (!checkUserPermissions(userIdv, U_CARER, U_CLINICIAN, U_ADMIN))
			{
				return l;
			}
			...
			try
			{
				...
				SystemDictionary.webguiLog("DEBUG", fromDate.toGregorianCalendar().getTime().toString());
				SystemDictionary.webguiLog("DEBUG", toDate.toGregorianCalendar().getTime().toString());
				SystemDictionary.webguiLog("DEBUG", "" + userId);
