...
public class FQuestionnaires
{
			...
			Integer parentId, SystemParameter locale) throws HibernateException, LocaleException
	{
		...
		SystemDictionary.webguiLog("DEBUG", "updateQuestionnaireQuestion : param 1 :   " + dQuestionnaireQuestion.getType());
		try
		{
			dQuestionnaireQuestion.setId(new Integer(xQuestionnaireQuestion.getId()));
			SystemDictionary.webguiLog("DEBUG", "updateQuestionnaireQuestion : param 2 :   " + dQuestionnaireQuestion.getId());
		}
		catch (Exception e)
		{
			SystemDictionary.webguiLog("ERROR", "updateQuestionnaireQuestion : xQuestionnaireQuestion.getId() is not a valid number : " + e.getMessage());
