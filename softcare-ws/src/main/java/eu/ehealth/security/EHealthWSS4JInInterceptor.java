...
public class EHealthWSS4JInInterceptor extends AbstractWSS4JInterceptor
{
    ...
    public static final String SECURITY_PROCESSED = EHealthWSS4JInInterceptor.class.getName() + ".DONE";
    
    private static final Logger LOG = LogUtils.getL7dLogger(EHealthWSS4JInInterceptor.class);
    private boolean ignoreActions;

    /**
     *
     */
    private EHealthWSSecurityEngine secEngineOverride;
    
    public EHealthWSS4JInInterceptor() {
        super();

        setPhase(Phase.PRE_PROTOCOL);
        getAfter().add(SAAJInInterceptor.class.getName());
    }
    public EHealthWSS4JInInterceptor(boolean ignore) {
        this();
        ignoreActions = ignore;
    }

    public EHealthWSS4JInInterceptor(Map<String, Object> properties) {
        this();
    ...
    public void handleMessage(SoapMessage msg) throws Fault {
        ...
        EHealthWSSecurityEngine engine;
        if (config != null) {
            engine = new EHealthWSSecurityEngine();
     ...
     *              base class).
     */
    protected EHealthWSSecurityEngine getSecurityEngine(boolean utWithCallbacks) {
        ...
        if (!utWithCallbacks) {
            ...
            profiles.put(EHealthWSSecurityEngine.USERNAME_TOKEN, validator);
     ...
     */
    protected static EHealthWSSecurityEngine
    createSecurityEngine(
        ...
        final EHealthWSSecurityEngine ret = new EHealthWSSecurityEngine();
    ...
    static class CXFRequestData extends RequestData {
        ...
        public Validator getValidator(QName qName) throws WSSecurityException {
            String key = null;
            if (EHealthWSSecurityEngine.SAML_TOKEN.equals(qName)) {
                key = SecurityConstants.SAML1_TOKEN_VALIDATOR;
            } else if (EHealthWSSecurityEngine.SAML2_TOKEN.equals(qName)) {
                key = SecurityConstants.SAML2_TOKEN_VALIDATOR;
            } else if (EHealthWSSecurityEngine.USERNAME_TOKEN.equals(qName)) {
                key = SecurityConstants.USERNAME_TOKEN_VALIDATOR;
            } else if (EHealthWSSecurityEngine.SIGNATURE.equals(qName)) {
                key = SecurityConstants.SIGNATURE_TOKEN_VALIDATOR;
            } else if (EHealthWSSecurityEngine.TIMESTAMP.equals(qName)) {
                key = SecurityConstants.TIMESTAMP_TOKEN_VALIDATOR;
            } else if (EHealthWSSecurityEngine.BINARY_TOKEN.equals(qName)) {
                key = SecurityConstants.BST_TOKEN_VALIDATOR;
            } else if (EHealthWSSecurityEngine.SECURITY_CONTEXT_TOKEN_05_02.equals(qName)
                || EHealthWSSecurityEngine.SECURITY_CONTEXT_TOKEN_05_12.equals(qName)) {
                ...
                try {
                    if (o instanceof Validator) {
                        ...
                        return (Validator)ClassLoaderUtils.loadClass(o.toString(),
                        											EHealthWSS4JInInterceptor.class)
                                                                     .newInstance();
                    }
