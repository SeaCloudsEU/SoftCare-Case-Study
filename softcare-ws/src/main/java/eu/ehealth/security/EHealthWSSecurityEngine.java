...
public class EHealthWSSecurityEngine
{


	private static org.apache.commons.logging.Log log = org.apache.commons.logging.LogFactory
			.getLog(WSSecurityEngine.class);

	/**
	 * The WSSConfig instance used by this SecurityEngine to find Processors for
	 ...
	 * specification
	 */
	public static final QName DERIVED_KEY_TOKEN_05_02 = new QName(
			ConversationConstants.WSC_NS_05_02,
			ConversationConstants.DERIVED_KEY_TOKEN_LN);

	/**
	 * <code>wsc:SecurityContextToken</code> as defined by WS-SecureConversation
	 * specification
	 */
	public static final QName SECURITY_CONTEXT_TOKEN_05_02 = new QName(
			ConversationConstants.WSC_NS_05_02,
	 ...
	 */
	public static final QName DERIVED_KEY_TOKEN_05_12 = new QName(
	 ...
	 * specification in WS-SX
	 */
	public static final QName SECURITY_CONTEXT_TOKEN_05_12 = new QName(
			ConversationConstants.WSC_NS_05_12,
			ConversationConstants.SECURITY_CONTEXT_TOKEN_LN);
	 ...
	 * 
	 * @return the WSSConfig instance previously set on this WSSecurityEngine
	 *         instance
	 */
	 ...
	 * <p/>
	 * This is the main entry point to verify or decrypt a SOAP envelope. First
	 * check if a <code>wsse:Security</code> is available with the defined
	 * actor.
	 * 
	 ...
	 * @param crypto the object that implements the access to the keystore and
	 *            the handling of certificates.
	...
	public List<WSSecurityEngineResult> processSecurityHeader(Document doc,
			String actor, CallbackHandler cb, Crypto crypto)
			throws WSSecurityException
	{
	 ...
	 * @param cb a callback hander to the caller to resolve passwords during
	 *            encryption and UsernameToken handling
	 * @param sigCrypto the object that implements the access to the keystore
	 *            and the handling of certificates for Signature
	 * @param decCrypto the object that implements the access to the keystore
	 *            and the handling of certificates for Decryption
	 * @return a result list
	 * @throws WSSecurityException
	 * @see WSSecurityEngine#processSecurityHeader(Element securityHeader,
	 *      CallbackHandler cb, Crypto sigCrypto, Crypto decCrypto)
	 */
	public List<WSSecurityEngineResult> processSecurityHeader(Document doc,
			String actor, CallbackHandler cb, Crypto sigCrypto, Crypto decCrypto)
			throws WSSecurityException
	{
	 ...
	 * Processors have been registered with the WSSCondig instance set on this
	 * class.
	 * 
	 ...
	 *            encryption and UsernameToken handling
	 * @param sigCrypto the object that implements the access to the keystore
	 *            and the handling of certificates used for Signature
	 * @param decCrypto the object that implements the access to the keystore
	 *            and the handling of certificates used for Decryption
	 * @return a List of {@link WSSecurityEngineResult}. Each element in the the
	 *         List represents the result of a security action. The elements are
	 *         ordered according to the sequence of the security actions in the
	 *         wsse:Signature header. The List may be empty if no security
	 ...
	 *            be able to provide the callback handler, cryptos, etc... as
	 ...
	 */
	public List<WSSecurityEngineResult> processSecurityHeader(
			Element securityHeader, RequestData requestData)
			throws WSSecurityException
	{
		...
		while (node != null)
		{
			...
			if (Node.ELEMENT_NODE == node.getNodeType())
			{
				...
				if (p != null)
				{
					...
					SystemDictionary.webguiLog("DEBUG", "Element                     : " + node.getLocalName() + " " + node.getNamespaceURI());
					SystemDictionary.webguiLog("DEBUG", "Element value               : " + node.getNodeValue());
					SystemDictionary.webguiLog("DEBUG", "Element getTextContent      : " + node.getTextContent());
					SystemDictionary.webguiLog("DEBUG", "Element toString            : " + node.toString());
					SystemDictionary.webguiLog("DEBUG", "requestData getSignatureUser: " + requestData.getSignatureUser());
					SystemDictionary.webguiLog("DEBUG", "wsDocInfo                   :" + wsDocInfo.toString());
					SystemDictionary.webguiLog("DEBUG", "wsDocInfo getSecurityHeader :" + wsDocInfo.getSecurityHeader().getNodeName());
				...
				else
				{
					if (doDebug)
					{
						log.debug("Unknown Element: " + node.getLocalName()
								+ " " + node.getNamespaceURI());
					}
				}
			}
			//
			// If the next sibling is null and the stored next sibling is not
			// null, then we have
			// encountered an EncryptedData element which was decrypted, and so
			// the next sibling
