	...
	public static OperationResult getOpResult(String code, String txtDesc) 
	{
		...
		try {
			...
			try {
				...
				SystemDictionary.webguiLog("ERROR", "Setting OperationResult values ERROR (1) : " + ex.getMessage());
			}
			...
			SystemDictionary.webguiLog("DEBUG", "RESPONSE - OperationResult --------------");
			SystemDictionary.webguiLog("DEBUG", "	C: " + res.getCode());
			SystemDictionary.webguiLog("DEBUG", "	D: " + res.getDescription());
			SystemDictionary.webguiLog("DEBUG", "	S: " + res.getStatus());
		}
		catch (Exception ex) {
			SystemDictionary.webguiLog("ERROR", "Setting OperationResult values ERROR (2) : " + ex.getMessage());
		}
